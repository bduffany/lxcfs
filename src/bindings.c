...
bool liblxcfs_functional(void) { return reload_successful != 0; }

bool liblxcfs_can_use_swap(void) { return can_use_swap; }

bool liblxcfs_can_use_sys_cpu(void) { return can_use_sys_cpu; }

bool liblxcfs_has_versioned_opts(void) { return has_versioned_opts; }

bool liblxcfs_memory_is_cgroupv2(void) { return memory_is_cgroupv2; }

__u32 liblxcfs_personality(void) { return host_personality; }
...
static inline void store_lock(void) { mutex_lock(&pidns_store_mutex); }

static inline void store_unlock(void) { mutex_unlock(&pidns_store_mutex); }
...
static int initpid_still_valid_pidfd(struct pidns_init_store *entry) {
  ...
  if (entry->init_pidfd < 0) return ret_errno(ENOSYS);

  ret = pidfd_send_signal(entry->init_pidfd, 0, NULL, 0);
  if (ret < 0) {
    if (errno == ENOSYS) return ret_errno(ENOSYS);
...
static int initpid_still_valid_stat(struct pidns_init_store *entry) {
  ...
  if (stat(path, &st) || entry->ctime != st.st_ctime) return 0;
...
static bool initpid_still_valid(struct pidns_init_store *entry) {
  ...
  if (ret < 0) ret = initpid_still_valid_stat(entry);
...
static void prune_initpid_store(void) {
  ...
  now = time(NULL);
  if (now < (last_prune + PURGE_SECS)) return;
  ...
  for (int i = 0; i < PIDNS_HASH_SIZE; i++) {
    for (struct pidns_init_store *entry = pidns_hash_table[i], *prev = NULL;
         entry;) {
      if (entry->lastcheck < threshold) {
        struct pidns_init_store *cur = entry;

        lxcfs_debug("Removed cache entry for pid %d to init pid cache",
                    cur->initpid);
...
static void save_initpid(ino_t pidns_inode, pid_t pid) {
  ...
  *entry = (struct pidns_init_store){
  ...
  lxcfs_debug("Added cache entry %d for pid %d to init pid cache", ino_hash,
              pid);
}
...
static void write_task_init_pid_exit(int sock, pid_t target) {
  ...
  if (fd < 0) log_exit("write_task_init_pid_exit open of ns/pid");
  ...
  if (pid == 0) {
    if (!send_creds_ok(sock)) _exit(EXIT_FAILURE);

    _exit(EXIT_SUCCESS);
  }

  if (!wait_for_pid(pid)) _exit(EXIT_FAILURE);
...
static pid_t scm_init_pid(pid_t task) {
  ...
  if (socketpair(AF_UNIX, SOCK_DGRAM, 0, sock) < 0) return -1;
  ...
  if (!recv_creds(sock[1], &cred, &v)) goto out;

  pid_ret = cred.pid;
  lxcfs_info(">>> scm_init_pid(%d) => %d", task, pid_ret);
...
pid_t lookup_initpid_in_store(pid_t pid) {
  ...
  lxcfs_info(">>> Looking up pid namespace %s", path);
  if (stat(path, &st)) return ret_errno(ESRCH);

  store_lock();

  hashed_pid = lookup_verify_initpid(st.st_ino);
  lxcfs_info("lookup_verify_initpid(ino=%ld) => %d", st.st_ino, hashed_pid);
  if (hashed_pid < 0) {
    ...
    if (hashed_pid > 0) {
      lxcfs_info(
          ">>> lookup_initpid_in_store(%d): calling save_initpid(ino=%ld, "
          "hashed_pid=%d)",
          pid, st.st_ino, hashed_pid);
...
static bool is_on_ramfs(void) {
  ...
  while (getline(&line, &len, f) != -1) {
    ...
    for (p = line, i = 0; p && i < 4; i++) p = strchr(p + 1, ' ');
    if (!p) continue;
    ...
    *p2 = '\0';
    if (strcmp(p + 1, "/") == 0) {
      ...
      p = strchr(p2 + 1, '-');
      if (p && strncmp(p, "- rootfs rootfs ", 16) == 0) return true;
    }
  }
...
static int pivot_enter(void) {
  ...
  if (oldroot < 0)
    return log_error_errno(-1, errno, "Failed to open old root for fchdir");

  newroot = open(ROOTDIR, O_DIRECTORY | O_RDONLY | O_CLOEXEC);
  if (newroot < 0)
    return log_error_errno(-1, errno, "Failed to open new root for fchdir");

  /* change into new root fs */
  if (fchdir(newroot) < 0)
    return log_error_errno(
        -1, errno, "Failed to change directory to new rootfs: %s", ROOTDIR);

  /* pivot_root into our new root fs */
  if (pivot_root(".", ".") < 0)
    return log_error_errno(-1, errno, "pivot_root() syscall failed: %s",
                           strerror(errno));
...
static int permute_and_enter(void) {
   ...
   * /proc/1/mountinfo. */
  if (has_fs_type(&sb, RAMFS_MAGIC) || is_on_ramfs()) return chroot_enter();
...
static bool permute_root(void) {
  /* Prepare new root. */
  if (permute_prepare() < 0) return false;

  /* Pivot into new root. */
  if (permute_and_enter() < 0) return false;
...
static bool cgfs_mount_hierarchies(void) {
  if (!mkdir_p(BASEDIR DEFAULT_CGROUP_MOUNTPOINT, 0755)) return false;

  if (!cgroup_ops->mount(cgroup_ops, BASEDIR)) return false;
...
static bool cgfs_setup_controllers(void) {
  if (!cgfs_prepare_mounts()) return false;
...
static void sigusr2_toggle_virtualization(int signo, siginfo_t *info,
                                          void *extra) {
  int ret;
  ...
  if (reload_successful) {
    ...
    ret = write(STDERR_FILENO, "Switched into non-virtualization mode\n",
                STRLITERALLEN("Switched into non-virtualization mode\n"));
    if (ret < 0) goto please_compiler;
  } else {
    reload_successful = 1;
...
static void __attribute__((constructor)) lxcfs_init(void) {
  __do_close int init_ns = -EBADF, root_fd = -EBADF, pidfd = -EBADF;
  int i = 0;
  pid_t pid;
  ...
  if (setns(init_ns, 0) < 0) {
    log_exit("%s - Failed to switch back to initial mount namespace",
             strerror(errno));
    goto broken_upgrade;
  }
  ...
  lxcfs_info(">>> reload_successful = 1");
  reload_successful = 1;
  return;
