...
struct cg_proc_stat {
  ...
  struct cpuacct_usage
      *usage; /* Real usage as read from the host's /proc/stat. */
...
static void free_proc_stat_node(struct cg_proc_stat *node) {
  if (node) {
     ...
     */
    if (node->usage) pthread_mutex_destroy(&node->lock);
    free_disarm(node->cg);
...
static struct cg_proc_stat *new_proc_stat_node(struct cpuacct_usage *usage,
  ...
  if (pthread_mutex_init(&node->lock, NULL)) return NULL;
  /*
   * We're abusing the usage pointer to indicate that
   * pthread_mutex_init() was successful. Don't judge me.
   */
...
static struct cg_proc_stat *prune_proc_stat_list(struct cg_proc_stat *node) {
  ...
  for (struct cg_proc_stat *prev = NULL; node;) {
    if (!cgroup_supports("cpu", node->cg, "cpu.shares")) {
       ...
       */
      if (pthread_mutex_trylock(&cur->lock)) goto next;
...
static void prune_proc_stat_history(void) {
  ...
  for (int i = 0; i < CPUVIEW_HASH_SIZE; i++) {
    ...
    if (proc_stat_history[i]->next) {
      proc_stat_history[i]->next =
          prune_proc_stat_list(proc_stat_history[i]->next);
...
static struct cg_proc_stat *find_or_create_proc_stat_node(
    struct cpuacct_usage *usage, int cpu_count, const char *cg) {
  ...
  if (node->cpu_count < cpu_count) {
    ...
    if (!expand_proc_stat_node(node, cpu_count)) {
      ...
      return log_debug(NULL, "Unable to expand stat node %d->%d for %s",
...
static void add_cpu_usage(uint64_t *surplus, struct cpuacct_usage *usage,
  ...
  if (free_space > usage->idle) free_space = usage->idle;
 ...
 * through `value`.
 */
static bool read_cpu_cfs_param(const char *cg, const char *param,
                               int64_t *value) {
  ...
  if (ret < 0 || (size_t)ret >= sizeof(file)) return false;

  if (!cgroup_ops->get(cgroup_ops, "cpu", cg, file, &str)) return false;
...
static double exact_cpu_count(const char *cg) {
  ...
  if (!read_cpu_cfs_param(cg, "quota", &cfs_quota)) return 0;

  if (!read_cpu_cfs_param(cg, "period", &cfs_period)) return 0;

  if (cfs_quota <= 0 || cfs_period <= 0) return 0;
...
static bool cfs_quota_disabled(const char *cg) {
  ...
  if (!read_cpu_cfs_param(cg, "quota", &cfs_quota)) return true;
...
int max_cpu_count(const char *cg) {
  ...
  if (!read_cpu_cfs_param(cg, "quota", &cfs_quota)) cfs_quota = 0;

  if (!read_cpu_cfs_param(cg, "period", &cfs_period)) cfs_period = 0;

  cpuset = get_cpuset(cg);
  if (cpuset) nr_cpus_in_cpuset = cpu_number_in_cpuset(cpuset);

  if (cfs_quota <= 0 || cfs_period <= 0) {
    if (nr_cpus_in_cpuset > 0) return nr_cpus_in_cpuset;
   ...
   * the remainder.
   */
  if ((cfs_quota % cfs_period) > 0) rv += 1;

  nprocs = get_nprocs();
  if (rv > nprocs) rv = nprocs;

  /* Use min value in cpu quota and cpuset. */
  if (nr_cpus_in_cpuset > 0 && nr_cpus_in_cpuset < rv) rv = nr_cpus_in_cpuset;
...
int cpuview_proc_stat(const char *cg, const char *cpuset,
  ...
  if (cg_cpu_usage_size < nprocs) nprocs = cg_cpu_usage_size;
  ...
  while (getline(&line, &linelen, f) != -1) {
    ...
    if (sscanf(line, "cpu%9[^ ]", cpu_char) != 1) break;

    if (sscanf(cpu_char, "%d", &physcpu) != 1) continue;

    if (physcpu >= cg_cpu_usage_size) continue;

    curcpu++;
    cpu_cnt++;

    if (!cpu_in_cpuset(physcpu, cpuset)) {
      for (i = curcpu; i <= physcpu; i++) cg_cpu_usage[i].online = false;
      continue;
    }
    ...
    ret =
        sscanf(line,
               ...
               " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 " %" PRIu64 "lu",
    ...
    if (all_used >= cg_used) {
      ...
      lxcfs_v("cpu%d from %s has unexpected cpu time: %" PRIu64
              " in /proc/stat, %" PRIu64
  ...
  if (max_cpus > cpu_cnt || !max_cpus) max_cpus = cpu_cnt;
  ...
  if (!diff) goto out_pthread_mutex_unlock;
  ...
  for (curcpu = 0; curcpu < nprocs; curcpu++) {
    if (!cg_cpu_usage[curcpu].online) continue;
  ...
  for (curcpu = 0, i = -1; curcpu < nprocs; curcpu++) {
    ...
    if (!stat_node->usage[curcpu].online) continue;
  ...
  if (max_cpus > 0) {
    ...
    for (curcpu = 0, i = -1; curcpu < nprocs; curcpu++) {
      ...
      if (diff[curcpu].user + diff[curcpu].system >= threshold) continue;

      /* Add user */
      add_cpu_usage(&user_surplus, &diff[curcpu], &diff[curcpu].user,
                    threshold);

      if (diff[curcpu].user + diff[curcpu].system >= threshold) continue;

      /* If there is still room, add system */
      add_cpu_usage(&system_surplus, &diff[curcpu], &diff[curcpu].system,
                    threshold);
    }
    ...
    for (curcpu = 0, i = -1; curcpu < nprocs; curcpu++) {
      ...
      lxcfs_v("curcpu: %d, diff_user: %" PRIu64 ", diff_system: %" PRIu64
              ", diff_idle: %" PRIu64 "\n",
              curcpu, diff[curcpu].user, diff[curcpu].system,
              diff[curcpu].idle);
    }
    lxcfs_v("total. diff_user: %" PRIu64 ", diff_system: %" PRIu64
            ", diff_idle: %" PRIu64 "\n",
    ...
    if (!cfs_quota_disabled(cg) && exact_cpus < (double)max_cpus) {
      uint64_t delta =
          (uint64_t)((double)(diff_user + diff_system + diff_idle) *
                     (1 - exact_cpus / (double)max_cpus));
  ...
  There will be N rendered cpus, indexed from 0 to N-1, cpu times of the cpus
  are calculated from those formula:
  - user_time[0] = stat_node->view[0].user + stat_node->view[1].user + ... +
  stat_node->view[a1].user
  - user_time[1] = stat_node->view[a1+1].user + stat_node->view[a1+1].user + ...
  + stat_node->view[a2].user
  ...
  - user_time[N-2] = stat_node->view[a(N-2)+1].user +
  stat_node->view[a(N-2)+2].user + ...
                      + stat_node->view[a(N-1)].user
  - user_time[N-1] = stat_node->view[a(N-1)+1].user +
  stat_node->view[a(N-1)+2].user + ...
                      + stat_node->view[aN] + ... + stat_node->view[K-1] (sum of
  all remaining CPUs)

  Similar formula applied for system and idle time
  */

  uint64_t curcpu_view_user_sum = 0, curcpu_view_system_sum = 0,
  ...
  for (curcpu = 0, i = -1; curcpu < nprocs; curcpu++) {
    ...
    l = snprintf(buf, buf_size,
                 ...
                 curcpu_view_user_sum, curcpu_view_system_sum,
...
out_pthread_mutex_unlock:
  if (stat_node) pthread_mutex_unlock(&stat_node->lock);
...
int proc_cpuinfo_read(char *buf, size_t size, off_t offset,
  ...
  lxcfs_info(">>> proc_cpuinfo_read: offset=%ld", offset);
  ...
  lxcfs_info(
      ">>> proc_cpuinfo_read: lookup_initpid_in_store(fc->pid: %d) returned %d",
      fc->pid, initpid);
  if (initpid <= 1 || is_shared_pidns(initpid)) initpid = fc->pid;

  cg = get_pid_cgroup(initpid, "cpuset");
  lxcfs_info(">>> proc_cpuinfo_read: initpid=%d, cg=%s", initpid, cg);
  if (!cg) return read_file_fuse("proc/cpuinfo", buf, size, d);
  prune_init_slice(cg);

  cpuset = get_cpuset(cg);
  lxcfs_info(">>> proc_cpuinfo_read: cpuset=%s", cpuset);
  if (!cpuset) return 0;
  ...
  if (cgroup_ops->can_use_cpuview(cgroup_ops) && opts && opts->use_cfs)
    ...
    use_view = false;
  if (use_view) max_cpus = max_cpu_count(cg);
  ...
  while (getline(&line, &linelen, f) != -1) {
    ...
    if (strncmp(line, "# processors:", 12) == 0) continue;

    if (is_processor_line(line)) {
      if (use_view && max_cpus > 0 && (curcpu + 1) == max_cpus) break;
      ...
      if (!cpu_in_cpuset(cpu, cpuset)) continue;
  ...
  if (is_s390x) {
    ...
    if (l < 0 || (size_t)l >= cache_size) return 0;
...
int read_cpuacct_usage_all(char *cg, char *cpuset,
  ...
  if (!cgroup_ops->get(cgroup_ops, "cpuacct", cg, "cpuacct.usage_all",
    ...
    if (!cgroup_ops->get(cgroup_ops, "cpuacct", cg, "cpuacct.usage_percpu",
                         &usage_str))
      return -1;
    ...
    if (sscanf(usage_str, "cpu user system\n%n", &read_cnt) != 0)
      return log_error(-1,
                       "read_cpuacct_usage_all reading first line from "
    ...
    for (i = 0, j = 0; i < cpucount; i++) {
      ret = sscanf(usage_str + read_pos, "%d %" PRIu64 " %" PRIu64 "\n%n",
                   &cg_cpu, &cg_user, &cg_system, &read_cnt);

      if (ret == EOF) break;

      if (ret != 3)
        return log_error(
            -EINVAL, "Failed to parse cpuacct.usage_all line %s from cgroup %s",
...
static bool cpuview_init_head(struct cg_proc_stat_head **head) {
  ...
  if (pthread_rwlock_init(&h->lock, NULL)) return false;
...
bool init_cpuview(void) {
  ...
  for (i = 0; i < CPUVIEW_HASH_SIZE; i++) proc_stat_history[i] = NULL;

  for (i = 0; i < CPUVIEW_HASH_SIZE; i++) {
    if (!cpuview_init_head(&proc_stat_history[i])) goto err;
  }
...
err:
  for (i = 0; i < CPUVIEW_HASH_SIZE; i++) {
    if (proc_stat_history[i]) free_disarm(proc_stat_history[i]);
  }
...
void free_cpuview(void) {
  for (int i = 0; i < CPUVIEW_HASH_SIZE; i++)
    if (proc_stat_history[i]) cpuview_free_head(proc_stat_history[i]);
}
