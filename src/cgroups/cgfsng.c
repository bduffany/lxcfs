...
static void must_append_controller(char **klist, char **nlist, char ***clist,
  ...
  if (string_in_list(klist, entry) && string_in_list(nlist, entry)) return;
...
static struct hierarchy *cgfsng_get_hierarchy(struct cgroup_ops *ops,
  ...
  if (!ops->hierarchies) return NULL;
  ...
  for (i = 0; ops->hierarchies[i]; i++) {
    if (!controller) {
      ...
      continue;
    } else if (pure_unified_layout(ops) && strcmp(controller, "devices") == 0) {
      if (ops->unified->bpf_device_controller) return ops->unified;
      break;
    }
...
static bool controller_lists_intersect(char **l1, char **l2) {
  ...
  for (i = 0; l1[i]; i++) {
    if (string_in_list(l2, l1[i])) return true;
  }
...
static bool controller_list_is_dup(struct hierarchy **hlist, char **clist) {
  ...
  for (i = 0; hlist[i]; i++)
    if (controller_lists_intersect(hlist[i]->controllers, clist)) return true;
...
static char **cg_hybrid_get_controllers(char **klist, char **nlist, char *line,
   ...
   */
  if (strncmp(p, DEFAULT_CGROUP_MOUNTPOINT "/", 15) != 0) return NULL;
...
static char **cg_unified_get_controllers(const char *file) {
  ...
  lxcfs_info(">>> get_controllers %s", file);
  ...
  lxc_iterate_parts(tok, buf, sep) {
    ...
    lxcfs_info(">>> get_controllers: tok=%s", tok);
...
static struct hierarchy *add_hierarchy(struct hierarchy ***h, char **clist,
                                       char *mountpoint, char *base_path,
                                       int type) {
...
static bool cgfsng_mount(struct cgroup_ops *ops, const char *root) {
  ...
  if (!ops) return ret_set_errno(false, ENOENT);

  if (!ops->hierarchies) return true;
  ...
  for (int i = 0; ops->hierarchies[i]; i++) {
    ...
    if (dir_exists(controllerpath)) continue;

    ret = mkdir(controllerpath, 0755);
    if (ret < 0)
      log_error_errno(goto on_error, errno, "Error creating cgroup path: %s",
                      controllerpath);
...
static int cgfsng_num_hierarchies(struct cgroup_ops *ops) {
  ...
  if (!ops) return ret_set_errno(-1, ENOENT);

  if (!ops->hierarchies) return 0;
...
static bool cgfsng_get_hierarchies(struct cgroup_ops *ops, int n, char ***out) {
  ...
  if (!ops->hierarchies) return false;

  /* sanity check n */
  for (i = 0; i < n; i++)
    if (!ops->hierarchies[i]) return ret_set_errno(false, ENOENT);
...
static int cgfsng_get_memory_stats_fd(struct cgroup_ops *ops,
                                      const char *cgroup) {
  __do_free char *path = NULL;
  struct hierarchy *h;
...
static int cgfsng_get_memory_slabinfo_fd(struct cgroup_ops *ops,
                                         const char *cgroup) {
  __do_free char *path = NULL;
  struct hierarchy *h;
  ...
  if (faccessat(h->fd, "memory.kmem.slabinfo", F_OK, 0)) return -1;
...
static bool cgfsng_can_use_swap(struct cgroup_ops *ops) {
  ...
  if (is_unified_hierarchy(h)) {
    if (faccessat(h->fd, "memory.swap.max", F_OK, 0)) return false;

    if (faccessat(h->fd, "memory.swap.current", F_OK, 0)) return false;

    has_swap = true;
  } else {
    if (faccessat(h->fd, "memory.memsw.limit_in_bytes", F_OK, 0)) return false;

    if (faccessat(h->fd, "memory.memsw.usage_in_bytes", F_OK, 0)) return false;
...
static char *readat_cpuset(int cgroup_fd) {
  ...
  if (val && strcmp(val, "") != 0) return move_ptr(val);
...
static int cgfsng_get_cpuset_cpus(struct cgroup_ops *ops, const char *cgroup,
  ...
  if (!is_unified_hierarchy(h)) {
    ...
    lxcfs_debug("cgfs_get_cpuset_cpus: cgroup v1");
  } else {
    ret = CGROUP2_SUPER_MAGIC;
    lxcfs_debug("cgfs_get_cpuset_cpus: cgroup v2");
  }
  ...
  *value = NULL;
  ...
  if (cgroup_fd < 0) {
    lxcfs_debug("cgfs_get_cpuset_cpus: FAILED to open path %s", path);
    return -1;
  }

  v = readat_cpuset(cgroup_fd);
  if (v) {
    lxcfs_debug("cgfs_get_cpuset_cpus: readat_cpuset: %s", v);
    *value = v;
    return ret;
  }

  lxcfs_debug(
      "cgfs_get_cpuset_cpus: cpuset.cpus and cpuset.cpus.effective are both "
      "empty");
  ...
  for (;;) {
    ...
    if (fd < 0 || !is_cgroup_fd(fd)) return -1;
...
static bool cgfsng_can_use_cpuview(struct cgroup_ops *ops) {
  ...
  if (pure_unified_layout(ops)) return true;

  cpu = ops->get_hierarchy(ops, "cpu");
  if (!cpu || is_unified_hierarchy(cpu)) return false;

  cpuacct = ops->get_hierarchy(ops, "cpuacct");
  if (!cpuacct || is_unified_hierarchy(cpuacct)) return false;
...
static int cg_hybrid_init(struct cgroup_ops *ops) {
  ...
  if (!basecginfo) return ret_set_errno(-1, ENOMEM);

  ret = get_existing_subsystems(&klist, &nlist);
  if (ret < 0)
    return log_error_errno(
        -1, errno, "Failed to retrieve available legacy cgroup controllers");
  ...
  while (getline(&line, &len, f) != -1) {
    ...
    if (type == CGROUP2_SUPER_MAGIC && ops->unified) continue;
    ...
    if (ops->cgroup_layout == CGROUP_LAYOUT_UNKNOWN) {
      ...
      if (type == CGROUP_SUPER_MAGIC) ops->cgroup_layout = CGROUP_LAYOUT_HYBRID;
    ...
    controller_list =
        cg_hybrid_get_controllers(klist, nlist, line, type, &controllers);
    if (!controller_list && type == CGROUP_SUPER_MAGIC) continue;
    ...
    if (!mountpoint)
      log_error_errno(goto next, EINVAL,
                      "Failed parsing mountpoint from \"%s\"", line);

    if (type == CGROUP_SUPER_MAGIC)
      base_cgroup = cg_hybrid_get_current_cgroup(basecginfo, controller_list[0],
                                                 CGROUP_SUPER_MAGIC);
    else
      base_cgroup =
          cg_hybrid_get_current_cgroup(basecginfo, NULL, CGROUP2_SUPER_MAGIC);
    if (!base_cgroup)
      log_error_errno(goto next, EINVAL, "Failed to find current cgroup %s",
                      mountpoint);
    ...
    if (type == CGROUP2_SUPER_MAGIC) {
      ...
      cgv2_ctrl_path =
          must_make_path(mountpoint, base_cgroup, "cgroup.controllers", NULL);
    ...
    new = add_hierarchy(&ops->hierarchies, controller_list, mountpoint,
                        base_cgroup, type);
    new->__controllers = move_ptr(controllers);
    if (type == CGROUP2_SUPER_MAGIC && !ops->unified) ops->unified = new;
...
static int cg_unified_init(struct cgroup_ops *ops) {
  ...
  lxcfs_info(">>> cg_unified_init");

  ret = unified_cgroup_hierarchy();
  if (ret == -ENOMEDIUM) return ret_errno(ENOMEDIUM);

  if (ret != CGROUP2_SUPER_MAGIC) return 0;

  base_cgroup = cg_unified_get_current_cgroup(1);

  lxcfs_info(">>> cg_unified_init: base_cgroup(pid: 1) => %s", base_cgroup);

  if (!base_cgroup) return ret_errno(EINVAL);
  prune_init_scope(base_cgroup);
  ...
  new = add_hierarchy(&ops->hierarchies, delegatable, mountpoint, base_cgroup,
                      CGROUP2_SUPER_MAGIC);

  if (bpf_devices_cgroup_supported()) new->bpf_device_controller = 1;

  ops->cgroup_layout = CGROUP_LAYOUT_UNIFIED;
  ops->unified = new;

  ops->cgroup2_root_fd =
      open(DEFAULT_CGROUP_MOUNTPOINT, O_DIRECTORY | O_PATH | O_CLOEXEC);
  if (ops->cgroup2_root_fd < 0) return -errno;
...
struct cgroup_ops *cgfsng_ops_init(void) {
  ...
  if (!cgfsng_ops) return ret_set_errno(NULL, ENOMEM);

  cgfsng_ops->cgroup_layout = CGROUP_LAYOUT_UNKNOWN;
  cgfsng_ops->mntns_fd = -EBADF;
  cgfsng_ops->cgroup2_root_fd = -EBADF;

  if (cg_init(cgfsng_ops)) return NULL;
