...
static off_t get_procfile_size(const char *path) {
  ...
  while ((sz = getline(&line, &len, f)) != -1) answer += sz;
...
__lxcfs_fuse_ops int proc_getattr(const char *path, struct stat *sb) {
  ...
  if (strcmp(path, "/proc/meminfo") == 0 ||
      strcmp(path, "/proc/cpuinfo") == 0 || strcmp(path, "/proc/uptime") == 0 ||
      strcmp(path, "/proc/stat") == 0 || strcmp(path, "/proc/diskstats") == 0 ||
      strcmp(path, "/proc/swaps") == 0 || strcmp(path, "/proc/loadavg") == 0 ||
...
__lxcfs_fuse_ops int proc_access(const char *path, int mask) {
  if (strcmp(path, "/proc") == 0 && access(path, R_OK) == 0) return 0;

  /* these are all read-only */
  if ((mask & ~R_OK) != 0) return -EACCES;
...
static uint64_t get_memlimit(const char *cgroup, bool swap) {
  ...
  if (ret > 0 && memlimit_str[0] &&
      safe_uint64(memlimit_str, &memlimit, 10) < 0)
...
static char *gnu_dirname(char *path) {
  ...
  if (last_slash != NULL && last_slash != path && last_slash[1] == '\0') {
    ...
    if (runp != path) last_slash = memrchr(path, '/', runp - path);
  }
...
static uint64_t get_min_memlimit(const char *cgroup, bool swap) {
  ...
  copy = strdup(cgroup);
  if (!copy) return log_error_errno(0, ENOMEM, "Failed to allocate memory");
  ...
  while (*copy && strcmp(copy, "/") != 0) {
    ...
    if (memlimit > 0 && memlimit < retlimit) retlimit = memlimit;
  };
  ...
  *memswpriority = 1;
  ...
  if (memswlimit > 0) {
    ret = cgroup_ops->get_memory_swap_current(cgroup_ops, cgroup,
                                              &memswusage_str);
    if (ret < 0 || safe_uint64(memswusage_str, &memswusage, 10) != 0) return;
    ...
    ret = cgroup_ops->get_memory_swappiness(cgroup_ops, cgroup,
                                            &memswpriority_str);
    if (ret >= 0) safe_uint64(memswpriority_str, memswpriority, 10);
  }
}
...
static int proc_swaps_read(char *buf, size_t size, off_t offset,
  ...
  __do_free char *cgroup = NULL, *memusage_str = NULL, *memswusage_str = NULL,
                 *memswpriority_str = NULL;
  struct fuse_context *fc = fuse_get_context();
  bool wants_swap =
      lxcfs_has_opt(fuse_get_context()->private_data, LXCFS_SWAP_ON);
  struct file_info *d = INTTYPE_TO_PTR(fi->fh);
  uint64_t memlimit = 0, memusage = 0, swtotal = 0, swusage = 0,
           memswpriority = 1, hostswtotal = 0, hostswfree = 0;
  ssize_t total_len = 0;
  ssize_t l = 0;
  char *cache = d->buf;
  int ret;
  ...
  if (offset) {
    ...
    if (offset > d->size) return -EINVAL;

    if (!d->cached) return 0;
  ...
  if (initpid <= 1 || is_shared_pidns(initpid)) initpid = fc->pid;

  cgroup = get_pid_cgroup(initpid, "memory");
  if (!cgroup) return read_file_fuse("/proc/swaps", buf, size, d);
  ...
  if (wants_swap)
    get_swap_info(cgroup, memlimit, memusage, &swtotal, &swusage,
                  &memswpriority);
  ...
  if (total_len < 0 || l < 0) return log_error(0, "Failed writing to cache");

  d->cached = 1;
  d->size = (int)total_len;

  if ((size_t)total_len > size) total_len = size;
...
static int proc_diskstats_read(char *buf, size_t size, off_t offset,
                               struct fuse_file_info *fi) {
  __do_free char *cg = NULL, *io_serviced_str = NULL, *io_merged_str = NULL,
                 *io_service_bytes_str = NULL, *io_wait_time_str = NULL,
                 *io_service_time_str = NULL, *line = NULL;
  __do_free void *fopen_cache = NULL;
  __do_fclose FILE *f = NULL;
  struct fuse_context *fc = fuse_get_context();
  struct file_info *d = INTTYPE_TO_PTR(fi->fh);
  ...
  uint64_t read_service_time, write_service_time, discard_service_time,
      read_wait_time, write_wait_time, discard_wait_time;
  char *cache = d->buf;
  ...
  if (!cg) return read_file_fuse("/proc/diskstats", buf, size, d);
  prune_init_slice(cg);
  ...
  while (getline(&line, &linelen, f) != -1) {
    ...
    get_blkio_io_value(io_serviced_str, stats.major, stats.minor, "Read",
                       &stats.read);
    get_blkio_io_value(io_serviced_str, stats.major, stats.minor, "Write",
                       &stats.write);
    get_blkio_io_value(io_serviced_str, stats.major, stats.minor, "Discard",
                       &stats.discard);

    get_blkio_io_value(io_merged_str, stats.major, stats.minor, "Read",
                       &stats.read_merged);
    get_blkio_io_value(io_merged_str, stats.major, stats.minor, "Write",
                       &stats.write_merged);
    get_blkio_io_value(io_merged_str, stats.major, stats.minor, "Discard",
                       &stats.discard_merged);

    get_blkio_io_value(io_service_bytes_str, stats.major, stats.minor, "Read",
                       &stats.read_sectors);
    stats.read_sectors = stats.read_sectors / 512;
    get_blkio_io_value(io_service_bytes_str, stats.major, stats.minor, "Write",
                       &stats.write_sectors);
    stats.write_sectors = stats.write_sectors / 512;
    get_blkio_io_value(io_service_bytes_str, stats.major, stats.minor,
                       "Discard", &stats.discard_sectors);
    stats.discard_sectors = stats.discard_sectors / 512;

    get_blkio_io_value(io_service_time_str, stats.major, stats.minor, "Read",
                       &read_service_time);
    read_service_time = read_service_time / 1000000;
    get_blkio_io_value(io_wait_time_str, stats.major, stats.minor, "Read",
                       &read_wait_time);
    read_wait_time = read_wait_time / 1000000;
    stats.read_ticks = read_service_time + read_wait_time;

    get_blkio_io_value(io_service_time_str, stats.major, stats.minor, "Write",
                       &write_service_time);
    write_service_time = write_service_time / 1000000;
    get_blkio_io_value(io_wait_time_str, stats.major, stats.minor, "Write",
                       &write_wait_time);
    write_wait_time = write_wait_time / 1000000;
    stats.write_ticks = write_service_time + write_wait_time;

    get_blkio_io_value(io_service_time_str, stats.major, stats.minor, "Discard",
                       &discard_service_time);
    discard_service_time = discard_service_time / 1000000;
    get_blkio_io_value(io_wait_time_str, stats.major, stats.minor, "Discard",
                       &discard_wait_time);
    discard_wait_time = discard_wait_time / 1000000;
    stats.discard_ticks = discard_service_time + discard_wait_time;

    get_blkio_io_value(io_service_time_str, stats.major, stats.minor, "Total",
                       &stats.total_ticks);
    ...
    if (stats.read || stats.write || stats.read_merged || stats.write_merged ||
        ...
        stats.rq_ticks || stats.discard_merged || stats.discard_sectors ||
        stats.discard_ticks)
      snprintf(lbuf, 256,
               "%u       %u" /* major, minor */
               " %s"         /* dev_name */
               ...
               "\n",
               stats.major, stats.minor, stats.dev_name, stats.read,
               stats.read_merged, stats.read_sectors, stats.read_ticks,
               stats.write, stats.write_merged, stats.write_sectors,
               stats.write_ticks, stats.ios_pgr, stats.total_ticks,
               stats.rq_ticks, stats.discard_merged, stats.discard_sectors,
               stats.discard_ticks);
    else
    ...
    if (l < 0) return log_error(0, "Failed to write cache");
...
static uint64_t get_reaper_start_time(pid_t pid) {
  ...
  if (qpid <= 0) return ret_errno(EINVAL);

  ret = snprintf(path, sizeof(path), "/proc/%d/stat", qpid);
  if (ret < 0 || (size_t)ret >= sizeof(path)) return ret_errno(EINVAL);

  f = fopen_cached(path, "re", &fopen_cache);
  if (!f) return ret_errno(EINVAL);
  ...
  ret = fscanf(f,
               ...
               &starttime);
  if (ret != 1) return ret_errno(EINVAL);
...
static int proc_uptime_read(char *buf, size_t size, off_t offset,
  ...
  if (reaperage >= busytime) idletime = reaperage - busytime;
...
static int proc_stat_read(char *buf, size_t size, off_t offset,
   ...
   */
  if (initpid == 1) return read_file_fuse("/proc/stat", buf, size, d);

  cg = get_pid_cgroup(initpid, "cpuset");
  if (!cg) return read_file_fuse("/proc/stat", buf, size, d);
  prune_init_slice(cg);
   ...
   */
  if (read_cpuacct_usage_all(cg, cpuset, &cg_cpu_usage, &cg_cpu_usage_size) ==
      0) {
    ...
    lxcfs_v(
        "proc_stat_read failed to read from cpuacct, falling back to the "
        "host's /proc/stat");
  }
  ...
  while (getline(&line, &linelen, f) != -1) {
    ...
    if (sscanf(cpu_char, "%d", &physcpu) != 1) continue;

    if (!cpu_in_cpuset(physcpu, cpuset)) continue;
    ...
    if (cg_cpu_usage) {
      if (physcpu >= cg_cpu_usage_size) break;

      all_used = user + nice + system + iowait + irq + softirq + steal + guest +
                 guest_nice;
      ...
      if (all_used >= cg_used) {
        ...
        lxcfs_debug("cpu%d from %s has unexpected cpu time: %" PRIu64
                    " in /proc/stat, %" PRIu64
  ...
  int cpuall_len =
      snprintf(cpuall, CPUALL_MAX_SIZE,
               "cpu "
               " %" PRIu64 /* user_sum */
               " %" PRIu64 /* nice_sum */
               " %" PRIu64 /* system_sum */
               ...
               "\n",
               user_sum, nice_sum, system_sum, idle_sum, iowait_sum, irq_sum,
               softirq_sum, steal_sum, guest_sum, guest_nice_sum);
...
static bool cgroup_parse_memory_stat(const char *cgroup,
  ...
  while ((linelen = getline(&line, &len, f)) != -1) {
    if (!unified && startswith(line, "hierarchical_memory_limit")) {
      ...
      sscanf(line, unified ? "shmem %" PRIu64 : "total_shmem %" PRIu64,
             &(mstat->total_shmem));
    } else if (startswith(line,
                          unified ? "file_mapped" : "total_mapped_file")) {
      sscanf(line,
             unified ? "file_mapped %" PRIu64 : "total_mapped_file %" PRIu64,
             &(mstat->total_mapped_file));
      ...
      sscanf(line, unified ? "pgfault %" PRIu64 : "total_pgfault %" PRIu64,
             &(mstat->total_pgfault));
    } else if (startswith(line, unified ? "pgmajfault" : "total_pgmajfault")) {
      sscanf(line,
             unified ? "pgmajfault %" PRIu64 : "total_pgmajfault %" PRIu64,
             &(mstat->total_pgmajfault));
    } else if (startswith(line,
                          unified ? "inactive_anon" : "total_inactive_anon")) {
      sscanf(
          line,
          unified ? "inactive_anon %" PRIu64 : "total_inactive_anon %" PRIu64,
          &(mstat->total_inactive_anon));
    } else if (startswith(line,
                          unified ? "active_anon" : "total_active_anon")) {
      sscanf(line,
             unified ? "active_anon %" PRIu64 : "total_active_anon %" PRIu64,
             &(mstat->total_active_anon));
    } else if (startswith(line,
                          unified ? "inactive_file" : "total_inactive_file")) {
      sscanf(
          line,
          unified ? "inactive_file %" PRIu64 : "total_inactive_file %" PRIu64,
          &(mstat->total_inactive_file));
    } else if (startswith(line,
                          unified ? "active_file" : "total_active_file")) {
      sscanf(line,
             unified ? "active_file %" PRIu64 : "total_active_file %" PRIu64,
             &(mstat->total_active_file));
    } else if (startswith(line,
                          unified ? "unevictable" : "total_unevictable")) {
      sscanf(line,
             unified ? "unevictable %" PRIu64 : "total_unevictable %" PRIu64,
             &(mstat->total_unevictable));
    }
  }
...
static int proc_meminfo_read(char *buf, size_t size, off_t offset,
  ...
  bool wants_swap =
      lxcfs_has_opt(fuse_get_context()->private_data, LXCFS_SWAP_ON);
  struct file_info *d = INTTYPE_TO_PTR(fi->fh);
  uint64_t memlimit = 0, memusage = 0, hosttotal = 0, swfree = 0, swusage = 0,
           swtotal = 0, memswpriority = 1;
  struct memory_stat mstat = {};
  size_t linelen = 0, total_len = 0;
  char *cache = d->buf;
  size_t cache_size = d->buflen;
  ...
  cgroup = get_pid_cgroup(initpid, "memory");
  if (!cgroup) return read_file_fuse("/proc/meminfo", buf, size, d);

  prune_init_slice(cgroup);

  /* memory limits */
  ret = cgroup_ops->get_memory_current(cgroup_ops, cgroup, &memusage_str);
  if (ret < 0) return read_file_fuse("/proc/meminfo", buf, size, d);
  ...
  f = fopen_cached("/proc/meminfo", "re", &fopen_cache);
  if (!f) return read_file_fuse("/proc/meminfo", buf, size, d);
  ...
  while (getline(&line, &linelen, f) != -1) {
    ...
    if (startswith(line, "MemTotal:")) {
      ...
      if (memlimit == 0) memlimit = hosttotal;

      if (hosttotal < memlimit) memlimit = hosttotal;
      ...
      if (wants_swap) {
        ...
        if (!liblxcfs_memory_is_cgroupv2()) swtotal += memlimit;
      ...
      snprintf(lbuf, 100, "AnonPages:      %8" PRIu64 " kB\n",
               (mstat.total_active_anon + mstat.total_inactive_anon -
                mstat.total_shmem) /
                   1024);
      printme = lbuf;
...
static int proc_slabinfo_read(char *buf, size_t size, off_t offset,
  ...
  cgroup = get_pid_cgroup(initpid, "memory");
  if (!cgroup) return read_file_fuse("/proc/slabinfo", buf, size, d);

  prune_init_slice(cgroup);

  fd = cgroup_ops->get_memory_slabinfo_fd(cgroup_ops, cgroup);
  if (fd < 0) return read_file_fuse("/proc/slabinfo", buf, size, d);

  f = fdopen_cached(fd, "re", &fopen_cache);
  if (!f) return read_file_fuse("/proc/slabinfo", buf, size, d);
...
static int proc_read_with_personality(
    int (*do_proc_read)(char *, size_t, off_t, struct fuse_file_info *),
  ...
  if (get_task_personality(fc->pid, &caller_personality) < 0)
    return log_error(0, "Failed to get caller process (pid: %d) personality",
                     fc->pid);
  ...
  lxcfs_info(">>> proc_read_with_personality: calling do_proc_read func");
...
__lxcfs_fuse_ops int proc_read(const char *path, char *buf, size_t size,
  ...
  switch (f->type) {
    case LXC_TYPE_PROC_MEMINFO:
      ...
      return read_file_fuse_with_offset(LXC_TYPE_PROC_MEMINFO_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_CPUINFO:
      lxcfs_info(">>> type = proc_cpuinfo");
      if (liblxcfs_functional()) {
        lxcfs_info(
            ">>> proc_read: liblxcfs_functional; calling "
            "proc_read_with_personality");
        int ret = proc_read_with_personality(&proc_cpuinfo_read, buf, size,
                                             offset, fi);
        lxcfs_info(">>> proc_read_with_personality returned %d", ret);
        return ret;
      }
      lxcfs_info(">>> proc_read: calling read_file_fuse_with_offset");
      return read_file_fuse_with_offset(LXC_TYPE_PROC_CPUINFO_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_UPTIME:
      if (liblxcfs_functional()) return proc_uptime_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_UPTIME_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_STAT:
      if (liblxcfs_functional()) return proc_stat_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_STAT_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_DISKSTATS:
      if (liblxcfs_functional())
        return proc_diskstats_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_DISKSTATS_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_SWAPS:
      if (liblxcfs_functional()) return proc_swaps_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_SWAPS_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_LOADAVG:
      if (liblxcfs_functional())
        return proc_loadavg_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_LOADAVG_PATH, buf, size,
                                        offset, f);
    case LXC_TYPE_PROC_SLABINFO:
      if (liblxcfs_functional())
        return proc_slabinfo_read(buf, size, offset, fi);

      return read_file_fuse_with_offset(LXC_TYPE_PROC_SLABINFO_PATH, buf, size,
                                        offset, f);
  }
