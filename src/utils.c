...
bool wait_for_sock(int sock, int timeout) {
  ...
  if ((starttime = time(NULL)) < 0) return false;

  epfd = epoll_create(1);
  if (epfd < 0) return log_error(false, "%m - Failed to create epoll socket");
...
again:
  if ((now = time(NULL)) < 0) return false;
...
static int msgrecv(int sockfd, void *buf, size_t len) {
  if (!wait_for_sock(sockfd, 2)) return -1;
...
int send_creds(int sock, struct ucred *cred, char v, bool pingfirst) {
  ...
  if (sendmsg(sock, &msg, 0) < 0) {
    if (errno == 3)
      return log_error(SEND_CREDS_NOTSK, "%s - Failed at sendmsg: %d",
                       strerror(errno), SEND_CREDS_NOTSK);

    return log_error(SEND_CREDS_FAIL, "%s - Failed at sendmsg: %d",
                     strerror(errno), SEND_CREDS_FAIL);
  }
...
int read_file_fuse(const char *path, char *buf, size_t size,
                   struct file_info *d) {
  ...
  if (d->size > (int)total_len) d->cached = d->size - total_len;
...
void prune_init_slice(char *cg) {
  ...
  if (cg_len < initscope_len) return;
...
FILE *fopen_cached(const char *path, const char *mode,
...
int safe_uint64(const char *numstr, uint64_t *converted, int base) {
  ...
  while (isspace(*numstr)) numstr++;

  if (*numstr == '-') return -EINVAL;

  errno = 0;
  u = strtoull(numstr, &err, base);
  if (errno == ERANGE && u == UINT64_MAX) return -ERANGE;

  if (err == numstr || *err != '\0') return -EINVAL;
...
int safe_uint32(const char *numstr, uint32_t *converted, int base) {
  ...
  if (errno == ERANGE && uli == UINT32_MAX) return -ERANGE;
...
char *read_file_at(int dfd, const char *fnam, unsigned int o_flags) {
  ...
  while ((linelen = getline(&line, &len, f)) != -1) {
    if (append_line(&buf, fulllen, line, linelen)) return NULL;
    fulllen += linelen;
  }
...
DIR *opendir_flags(const char *path, int flags) {
  ...
  dirp = fdopendir(dfd);
  if (dirp) move_fd(dfd); /* Transfer ownership to fdopendir(). */
